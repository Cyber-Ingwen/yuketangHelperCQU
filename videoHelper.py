...
cookie = '' # 你的cookie
csrftoken = cookie[cookie.index('csrftoken') + len('csrftoken='):]
csrftoken = csrftoken[:csrftoken.index(';')]
# sessionid = '' # expired
...
def one_video_watcher(video_id,video_name,cid,user_id,classroomid,skuid):
    ...
    get_url = "https://www.yuketang.cn/video-log/get_video_watch_progress/?cid="+str(cid)+"&user_id="+user_id+"&classroom_id="+classroomid+"&video_type=video&vtype=rate&video_id=" + str(video_id) + "&snapshot=1&term=latest&uv_id=3078"
    ...
    while val != "1.0" and val != '1':
        ...
        try:
            ...
            video_frame = 0
            print("恢复工作啦～～")
            submit_url = "https://www.yuketang.cn/mooc-api/v1/lms/exercise/problem_apply/?term=latest&uv_id=3078"
...
if __name__ == "__main__":
    # 获取userid
    response = requests.get('https://www.yuketang.cn/v2/api/web/userinfo', headers=headers).text
    user_id = re.search(r'"user_id":(.*?),', response).group(1)
    params = {
        'identity': '2',
    }
    response = requests.get('https://www.yuketang.cn/v2/api/web/courses/list', params=params,headers=headers).json()
    ...
    url = 'https://www.yuketang.cn/v2/api/web/logs/learn/%s?actype=-1&page=0&offset=20&sort=-1'% cid
    ...
    headers['xtbz'] = 'ykt'
    headers['Referer'] = 'https://www.yuketang.cn/v2/web/studentLog/%s'%cid
    if select:
        skuid = requests.get(url='https://www.yuketang.cn/v2/api/web/classrooms/%s?role=5' % cid,headers=headers).json()['data']['free_sku_id']
        get_url = 'https://www.yuketang.cn/c27/online_courseware/schedule/score_detail/single/%s/0/' % skuid
        ret = requests.get(url=get_url, headers=headers).json()
        for i in ret['data']['leaf_level_infos']:
            if i['leaf_type'] == leaf_type['homework']:
                continue
            get_url = 'https://www.yuketang.cn/mooc-api/v1/lms/learn/leaf_info/%s/%s/' % (cid, i['id'])
